	...
	foreach( $metaboxes as $key => $metabox) 
	{
		...
		?>			
			<tr <?php if(ecpt_is_odd($i)) { echo 'class="alternate"'; } ?> id="ecpt-metabox-<?php echo $metabox->id; ?>">
				<td><?php echo utf8_decode($metabox->nicename); ?></td>
				...
				<td>
					<?php
						foreach( $wpdb->get_results("SELECT nicename FROM " . $ecpt_db_meta_fields_name . " WHERE parent='" . $name . "';") as $key => $meta_field)	
						{
							echo utf8_decode($meta_field->nicename) . ', ';
						}
					?>
				</td>
				<td>
					...
					<a href="admin.php?page=easy-content-types/easy-content-types.php?metaboxes&ecpt-action=duplicate&metabox-id=<?php echo $metabox->id; ?>&metabox-name=<?php echo $metabox->name; ?>" title="<?php _e('Duplicate', 'ecpt'); ?>" class="ecpt-edit"><?php _e('Duplicate', 'ecpt'); ?></a> | 
