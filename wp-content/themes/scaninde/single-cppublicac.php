...
<div id="content">
	...
	<?php if ( have_posts() ) : while ( have_posts() ) : the_post(); ?>
			...
			<div id="content-page"> 
				<strong class="cat-title"><a href="/publicacoes-santuario/" title="Publicações do Santuário">Publicações do Santuário</a></strong>
				<h1 class="page-title"><?php the_title(); ?></h1>
				<?php include_once "includes/incl_compartilhe.php"; ?>
				<?php the_content(); // o conteudo do post 
				
				//Issuu
				if ( get_post_meta($post->ID, 'ecpt_pubcodhtml', true) ) :
					$metaissuu = get_post_meta($post->ID, 'ecpt_pubcodhtml', true);
					echo '<div class="meta-issuu">'.$metaissuu.'</div>';
				endif;
				
				//Jornal PDF para download
				if ( get_post_meta($post->ID, 'ecpt_arquivpub', true) ) :
					$metapdf = get_post_meta($post->ID, 'ecpt_arquivpub', true);
					echo '<div class="meta-download"><a href="'.$metapdf.'" title="Baixar Publicação">';
					the_title();
					echo '<span class="downloadinstr">Clique para baixar a publicação</span></a></div>';
				endif;
